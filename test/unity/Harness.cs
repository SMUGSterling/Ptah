...
static class Harness {
  ...
  static void Main(string[] a) {
    ...
    // 2: selecting the Yard group (below Root): the Yard spawn still resolves by its tail, Arena's is not under the selection
    top = Build(out s1, out s2, out gate, out start);
    Run(top.transform.children[0].children[2].gameObject);
    Ok(M(s2) != null && M(s1) == null, "selecting Yard converts only Yard/Spawn_01");
